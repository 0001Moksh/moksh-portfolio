  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col sm:flex-row items-center justify-center mb-12 space-y-4 sm:space-y-0 sm:space-x-4">
        ...
        <motion.button
          className="relative px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-full shadow-lg hover:shadow-xl transition-shadow flex items-center space-x-2"
      ...
      >
        {categories.map((category, catIndex) => (
          <motion.div
            ...
            className="w-full min-h-[280px] sm:min-h-[300px] flex items-center justify-center cursor-pointer"
          ...
          >
            ...
            >
              {/* Front Side */}
              <motion.div
                className="absolute w-full h-full bg-white border border-gray-300 rounded-2xl shadow-lg flex flex-col items-center justify-center p-4 sm:p-6 transition-all duration-300 group-hover:shadow-indigo-300 group-hover:border-indigo-400"
                style={{ backfaceVisibility: "hidden" }}
              >
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800 tracking-wide border-b-2 border-indigo-400 pb-2">
                  {category.title}
                </h2>
                <p className="text-gray-600 mt-4 text-xs sm:text-sm">Tap or click to explore skills</p>
              </motion.div>

              {/* Back Side */}
              <motion.div
                className="absolute w-full h-full bg-white border border-gray-300 rounded-2xl shadow-lg p-4 sm:p-6 overflow-auto transition-all duration-300 group-hover:shadow-indigo-300 group-hover:border-indigo-400"
                style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}
              >
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800 tracking-wide mb-4 border-b-2 border-indigo-400 pb-2">
                  {category.title}
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 place-items-center">
                  {category.skills.map((skill) => (
                    <motion.div
                      key={skill.id}
                      className="bg-white border border-gray-300 rounded-xl shadow-md p-3 sm:p-4 flex flex-col items-center justify-center w-full transition-all duration-300 hover:shadow-indigo-300 hover:border-indigo-400"
                      whileHover={{ scale: 1.05, rotate: 2 }}
                    >
                      <img
                        src={skill.logo}
                        alt={skill.name}
                        className="w-10 h-10 sm:w-[50px] sm:h-[50px] object-contain mb-2 rounded-full shadow-sm"
                        loading="lazy"
                      />
                      <h3 className="text-gray-800 font-semibold text-xs sm:text-sm text-center">
                        {skill.name}
                      </h3>
                    </motion.div>
                  ))}
